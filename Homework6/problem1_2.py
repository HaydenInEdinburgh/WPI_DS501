...
def compute_dz_dw(x):
    '''
        Compute partial gradients of the logit function gradients of the logit function z with respect to (w.r.t.) the weights w.
        Input:
            x: the feature vector of a data instance, a float numpy matrix of shape p by 1.
...
def check_dL_dw(x, y, w, b, delta=1e-7):
    ...
    '''
    i_range = x.shape[0]  # number of features
    dL_dw = np.mat(np.zeros(p)).T
    for i in range(i_range):
        d = np.mat(np.zeros(i_rang)).T
        d[i] = delta
        dL_dw[i] = (forward(x, y, w + d, b)[-1] -
...
def check_dL_db(x, y, w, b, delta=1e-7):
    ...
    '''
    a = (forward(x, y, w, b + delta)[-1] - forward(x, y, w, b)[-1])
    dL_db = a / delta
    return dL_db
