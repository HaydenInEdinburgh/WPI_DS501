...
def test_inference():
    ...
    c22.p = 'c5'
    c2.i = 0
    c2.C = {'apple': c21, 'orange': c22}
    x = np.array(['orange', 'low', 'bad', 'b'])
    y = Tree.inference(t, x)
    print(y)
    assert y == 'c5'
...
# -------------------------------------------------------------------------
def test_dataset1():
    '''(2 points) test_dataset1'''
    X, Y = Tree.load_dataset()
    t = Tree.train(X, Y)
    Y_predict = Tree.predict(t, X)
    accuracy = sum(Y == Y_predict) / \
        42.  # training accuracy of 42 training samples
