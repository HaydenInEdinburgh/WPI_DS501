...
def compute_mu_image(X):
    ...
    _,mu= p1.centering_X(X)
    mu_image = mu.reshape([64,64])
...
def load_dataset():
    ...
    load = fetch_olivetti_faces(data_home=None, shuffle=False, random_state=0, download_if_missing=True)
    X = np.mat(load.data)
    # the images
    images = load.images
    # the label to predict is the id of the person
    y = np.transpose(np.mat(load.target))
...
def compute_eigen_faces(X, k=20):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    Xp, P= p1.PCA(X,k) #Xp（400，k）P <-- (4096,k)
    P_images = list()
    for i in range(k):
        P_sin = P[:,i]
        P_centered = reshape_to_image(P_sin)
        P_images.append(P_centered)
