...
def elo_rating(W, n_player, K= 16.):
    ...
    for (A, B) in W:
        ...
        ##############################
        # INSERT YOUR CODE HERE
        RA = R[A] # player A's rating
        RB = R[B] # player B's rating
        EA = compute_EA(RA, RB) # the expected probability of (player A wins the game)
        R[A] = update_RA(RA, 1., EA, K) # update A's rating
        ##############################
